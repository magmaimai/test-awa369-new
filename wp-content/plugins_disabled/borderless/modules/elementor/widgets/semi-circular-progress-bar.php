...
class Semi_Circular_Progress_Bar extends Widget_Base {
	...
	protected function render() {
		...
		// Sanitize & escape Title to prevent stored XSS
		$raw_title   = $settings['borderless_elementor_semi_circular_progress_bar_title'];
		$clean_title = sanitize_text_field( $raw_title );
		$safe_title  = esc_js( $clean_title );

		// Stroke color style
		if ( 'borderless-elementor-semi-circular-progress-bar-stroke-solid-color' === $settings['borderless_elementor_semi_circular_progress_bar_stroke_color_style'] ) {
			$stroke_color_style = 'stroke_color_mode="solid" stroke_color="' . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_stroke_color'] ) . '"';
		} else {
			$stroke_color_style = 'stroke_color_mode="gradient" '
				. 'stroke_color="'      . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_stroke_color'] )      . '" '
				. 'stroke_color_from="' . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_stroke_color_from'] ) . '" '
				. 'stroke_color_to="'   . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_stroke_color_to'] )   . '"';
		}

		echo '<div class="borderless-elementor-semi-circular-progress-bar-widget ' . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_alignment'] ) . '">
			<div class="borderless-elementor-semi-circular-progress-bar"
			     title="'             . $safe_title                                                                    . '"
			     counter_value="'    . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_counter_value']['size'] )         . '"
			     '                  . $stroke_color_style                                                                    . '
			     trail_color="'      . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_trail_color'] )             . '"
			     counter_color="'    . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_counter_color'] )            . '"
			     stroke_width="'     . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_stroke_width']['size'] )    . '"
			     trail_width="'      . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_trail_width']['size'] )     . '"
			     animation_duration="' . esc_attr( $settings['borderless_elementor_semi_circular_progress_bar_animation_duration']['size'] ) . '">
			</div>
		</div>';
	}
