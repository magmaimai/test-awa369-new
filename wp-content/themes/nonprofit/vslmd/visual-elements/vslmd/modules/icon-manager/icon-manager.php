...
if ( ! class_exists( 'VE_Icon_Manager' ) ) {
	class VE_Icon_Manager {
		...
		function admin_scripts() {
			...
			wp_enqueue_script( 'vslmd-admin-media', get_template_directory_uri() . '/vslmd/visual-elements/vslmd/modules/icon-manager/assets/js/icon-manager.min.js', array( 'jquery' ) );
			wp_enqueue_script( 'media-upload' );
			wp_enqueue_media();
			wp_enqueue_style( 'vslmd-icon-manager-admin', get_template_directory_uri() . '/vslmd/visual-elements/vslmd/modules/icon-manager/assets/css/icon-manager.css' );
...
/*-----------------------------------------------------------------------------------*/
		...
		function icon_manager_dashboard() {
			...
			<div class="wrap">
				<h2>
					<?php esc_html_e( 'Icon Manager', 'vslmd' ); ?>
					...
					<?php esc_html_e( 'Upload New Icons', 'vslmd' ); ?>
					...
					<div class="postbox ve-postbox-search">
						...
						<div class="inside">
							<div class="search-area">
								<input class="search-icon" type="text" placeholder="<?php esc_attr_e( 'Start typing to search', 'vslmd' ); ?>"/>
		...
		<?php else: ?>
			<div class="error">
				<p>
					<?php esc_html_e( 'No font icons uploaded. Upload some font icons to display here.', 'vslmd' ); ?>
				</p>
			</div>
			<?php
			endif;
		...
		static function get_font_set() {
			...
			foreach ( $fonts as $font => $info ) {
				...
				if ( ! empty( $icon_set ) ) {
					...
					if ( $font == 'vslmd' ) {
						$output .= '<h3 class="icon_font_name"><strong>' . esc_html__( 'Default Icons', 'vslmd' ) . '</strong>';
					} else {
					...
					if ( $n != 1 ) {
						$output .= '<button class="button button-primary button-small vslmd_del_icon" data-delete=' . esc_attr( $font ) . ' data-title="' . esc_attr__( 'Delete Icon Set', 'vslmd' ) . '">' . esc_html__( 'Delete Icon Set', 'vslmd' ) . '</button>';
					}
					$output .= '</h3>';
			...
			jQuery(document).ready(function(){
				jQuery(".search-icon").keyup(function(){
					...
					jQuery(".icons-list li").each(function(){
						...
						if(count == 0)
							jQuery(".search-count").html(" ' . esc_js( __( 'Can\'t find the icon?', 'vslmd' ) ) . ' <a href=\'#vslmd_upload_icon\' class=\'button button-primary vslmd_upload_icon\' data-target=\'iconfont_upload\' data-title=\' ' . esc_js( __( 'Upload/Select Fontello Font Zip', 'vslmd' ) ) . ' \' data-type=\'application/octet-stream, application/zip\' data-button=\' ' . esc_js( __( 'Insert Fonts Zip File', 'vslmd' ) ) . ' \' data-trigger=\'vslmd_insert_zip\' data-class=\'media-frame\'>' . esc_js( __( 'Upload New Font icon', 'vslmd' ) ) . '</a>");
						else
							jQuery(".search-count").html(count+" ' . esc_js( __( 'Icons found.', 'vslmd' ) ) . '");
	...
	function add_zipped_font() {
		...
		if ( $this->font_name == 'unknown' ) {
			...
			die( esc_html__( 'Was not able to retrieve the Font name from your Uploaded Folder', 'vslmd' ) );
		}
		die( esc_html__( 'vslmd_font_added:', 'vslmd' ) . $this->font_name );
	}
	...
	function remove_zipped_font() {
		...
		if ( $delete ) {
			...
			die( esc_html__( 'vslmd_font_removed', 'vslmd' ) );
		}
		die( esc_html__( 'Was not able to remove Font', 'vslmd' ) );
	}
	...
	function zip_flatten( $zipfile, $filter ) {
		...
		if ( ! $tempdir ) {
			die( esc_html__( 'Wasn\'t able to create temp folder', 'vslmd' ) );
		}
		$zip = new ZipArchive;
		if ( $zip->open( $zipfile ) ) {
			for ( $i = 0; $i < $zip->numFiles; $i ++ ) {
				...
				if ( substr( $entry, - 1 ) == '/' || ! empty( $delete ) ) {
					...
					if ( ! $fp ) {
						die( esc_html__( 'Unable to extract the file.', 'vslmd' ) );
					}
					while ( ! feof( $fp ) ) {
				...
				die( esc_html__( "Wasn't able to work with Zip Archive", 'vslmd' ) );
			}
		...
		function create_config() {
			...
			if ( empty( $this->json_file ) || empty( $this->svg_file ) ) {
				$this->delete_folder( $this->paths['tempdir'] );
				die( esc_html__( 'selection.json or SVG file not found. Was not able to create the necessary config files', 'vslmd' ) );
			}
			...
			if ( ! is_wp_error( $json ) && ! empty( $json ) ) {
				...
				if ( ! isset( $file_contents->IcoMoonType ) ) {
					$this->delete_folder( $this->paths['tempdir'] );
					die( esc_html__( 'Uploaded font is not from IcoMoon. Please upload fonts created with the IcoMoon App Only.', 'vslmd' ) );
				}
			...
			if ( $handle ) {
				...
				foreach ( $this->json_config[ $this->font_name ] as $icon => $info ) {
					if ( ! empty( $info ) ) {
						...
						$this->delete_folder( $this->paths['tempdir'] );
						die( esc_html__( 'Was not able to write a config file', 'vslmd' ) );
					}
				}
				fclose( $handle );
			} else {
		...
		function re_write_css() {
			...
			if ( $file ) {
				...
				die( esc_html__( 'Unable to write css. Upload icons downloaded only from icomoon', 'vslmd' ) );
			}
		}
...
if ( ! function_exists( 'visual_elements_icon_manager_menu' ) ) {
	function visual_elements_icon_manager_menu() {
		$icon_manager_page = add_submenu_page(
			'visualmodo',
			esc_html__( "Visual Elements Icons", "vc_extend" ),
			esc_html__( "Visual Elements Icons", "vc_extend" ),
			"manage_options",
			"icon-manager",
